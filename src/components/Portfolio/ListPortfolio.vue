...
<template>
    ...
    <div v-for="(data, index) in portfolio" :key="index">
        <div class="flex flex-row gap-2 bg-backgroundColor-200 overflow-hidden rounded-2xl shadow-md hover:scale-[102%] transition-all duration-200 ease-in-out max-h-[200px] dark:bg-textColor">
            ...
            <div class="flex flex-col px-3 pt-2 pb-4">
                ...
                <h4 class="text-sm md:text-lg lg:text-2xl leading-none font-semibold text-primary dark:text-primary-200">{{ data.title
                    }}</h4>
